...
// 用户注册
function registerUser(username, mbti, gender, birthday, region) {
...
// 更新用户信息
function updateUser(userId, mbti, gender, birthday, region) {
    const userIndex = users.findIndex(u => u.id === userId);
    if (userIndex !== -1) {
        users[userIndex].mbti = mbti || '';
...
function handleRegister() {
    ...
    const newUser = registerUser(username, mbti, gender, birthday, region);
    ...
    if (newUser) {
        ...
        document.getElementById('regMbti').value = '';
...
function showUserProfile() {
    ...
    document.getElementById('profileMbti').value = currentUser.mbti || '';
...
function saveUserProfile() {
    ...
    const mbti = document.getElementById('profileMbti').value;
    const gender = document.getElementById('profileGender').value;
    const birthday = document.getElementById('profileBirthday').value;
    const region = document.getElementById('profileRegion').value.trim();
    
    updateUser(currentUser.id, mbti, gender, birthday, region);
